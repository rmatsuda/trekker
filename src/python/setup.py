...
class TrekkerBuildExt(build_ext):
    def run(self):
        ...
        target_dir = Path(self.build_lib) / "Trekker"
        os.makedirs(target_dir, exist_ok=True)

        for file in glob.glob("cython/*.pxd"):
            shutil.copy(file, target_dir)

        shutil.copyfile(
            "build/cython/Trekker.cpp",
            target_dir / "Trekker.cpp",
        )
...
cpp_sources = [
    str(i)
    for i in Path("../").glob("**/*.cpp")
    if not str(i).endswith("cmd.cpp")
    and not str(i).lower().endswith("trekker.cpp")
]

c_sources = [str(i) for i in Path("../").glob("**/*.c")]

sources = cpp_sources + c_sources
sources += ["cython/Trekker.pyx", "../trekker.cpp"]
